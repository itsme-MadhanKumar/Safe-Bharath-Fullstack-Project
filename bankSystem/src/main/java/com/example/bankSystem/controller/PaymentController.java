...
//                //-----Notify to the user
    ...
    {
        LoanDetails loan = service.findLoanByApplicationId(applicationId);
        if (loan != null) {
            return ResponseEntity.ok(Map.of(
                    "found", true,
                    "application_id", loan.getApplicationId(),
                    "name", loan.getName(),
                    "aadhaar_card", loan.getAadhaarCard(),
    ...
    public ResponseEntity<?> payLoan(@RequestBody Map<String, String> payload)
    {
        ...
        String currentBalanceofUser = service.CurrentBalanceService(userId);
        String checkMoneyCredentials = service.checkMoney(amount);
        String Loan_SanchendMoney = service.gettingLoanSancnedMoneyService(applicationId);
        // Parse strings to long
        if(amount.matches(".*\\d+\\.\\d+.*"))
        {
            return ResponseEntity.ok(Map.of("success", false, "message", "Decimal Values are Not Accepted."));
        }
        long amountToPay = Long.parseLong(amount);
        long currentBalance = Long.parseLong(currentBalanceofUser);
        long loan_amount = Long.parseLong(Loan_SanchendMoney);
...
//        mailSender.sending(email,otp);
    ...
    public ResponseEntity<Map<String, Object>> verifyOtp(@RequestBody OtpVerificationRequestPayLoan request) {
        ...
        {
            long fetchingActualLoan = Long.parseLong(service.CurrentActualLoanBalanceService(request.getUserId(),request.getApplicationId()));
            ...
            boolean UpdatingbankTable = service.deductingMoneyPayLoanService(request.getUserId(),request.getApplicationId(),String.valueOf(sub2));
            ...
            long making0inActualLoanifPossible = Long.parseLong(service.CurrentActualLoanBalanceService(request.getUserId(),request.getApplicationId()));

            if(making0inActualLoanifPossible==0)
            {
                service.setting_0_Service(request.getUserId(),request.getApplicationId(),"Loan Closed");
            }
            else
            {
                service.setting_0_Service(request.getUserId(), request.getApplicationId(),"Repayment Started");
            }
        }
        try{Thread.sleep(8000);} catch (InterruptedException e) {System.out.println("Thread interrupted: " + e.getMessage());}
        response.put("success", true);
